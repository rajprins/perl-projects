...
my $MAX_COLS = 10 ;   				# 10 cellen breed
my $MAX_ROWS  = 15 ;  				# 15 cellen hoog
my $TILE_WIDTH = 20;  				# breedte in pixels 
my $TILE_HEIGHT = 20; 				# hoogte in pixels 
...
my $interval = 500; 	                  # in milliseconden
my @heap = ();                            # heap voor tile IDs voor gevulde cellen
$heap[$MAX_COLS * $MAX_ROWS - 1] = undef; # grootte
...
sub fall {                 # Called when spacebar hit
    return if (!@cells);   # Return if not initialize
...
sub create_random_block {
    # choose a random pattern, a random color, and position the block at the top of the heap.
    ...
    my $cell;
    foreach $cell (@cells) {
        # If something already exists where the block is supposed to be, return false
...
sub create_screen {
    $w_top = MainWindow->new(-title =>'Caroline\'s Tetris');
    ...
    my $w_quit = $w_top->Button('-text' => 'Einde',
